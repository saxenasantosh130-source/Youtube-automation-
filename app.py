...
PEXELS_API_KEY = os.environ.get('PEXELS_API_KEY', 'ELlbnrqmKO7JH4ms1NElyzWcxZog5N4TsNqnmMBVj7p41ctxjnOakPq4')
...
<head>
    ...
    <style>
        ...
        .btn { padding: 14px; font-size: 15px; color: white; border: none; font-weight: bold; cursor: pointer; border-radius: 8px; flex: 1; text-align: center; }
    ...
    <div class="card">
        ...
        <form method="POST">
            ...
            <div class="btn-container">
                <button type="submit" name="action" value="download" class="btn btn-download">Download Video</button>
...
def make_story_video(topic, script_text):
    ...
    audio_filter = '-filter_complex "[0:a]volume=1.0[voice];[1:a]volume=0.15[music];[voice][music]amix=inputs=2:duration=first[aout]" -map 2:v -map "[aout]"'
    
    if has_video and os.path.exists("bg.mp4"):
        cmd = f'{ffmpeg_executable} -y -i voice.mp3 -stream_loop -1 -i bg_music.mp3 -stream_loop -1 -i bg.mp4 {audio_filter} -vf "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,drawtext=text=\'{topic}\':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -c:a aac -shortest final.mp4'
    else:
        cmd = f'{ffmpeg_executable} -y -i voice.mp3 -stream_loop -1 -i bg_music.mp3 -f lavfi -i color=c=black:s=1080x1920:r=25 {audio_filter} -vf "drawtext=text=\'{topic}\':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -c:a aac -shortest final.mp4'
    ...
    if request.method == 'POST':
        ...
        if action == 'generate_script':
            ...
            try:
                ...
                if action == 'download':
                    return send_file('final.mp4', as_attachment=True, download_name=f'{topic_val}_{dur_val}s.mp4')
