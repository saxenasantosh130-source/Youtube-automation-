...
def make_story_video(topic, script_text):
    ...
    if has_video and os.path.exists("bg.mp4"):
        cmd = f'ffmpeg -y -i voice.mp3 -stream_loop -1 -i bg_music.mp3 -stream_loop -1 -i bg.mp4 {audio_filter} -vf "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,drawtext=text=\'{topic}\':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -c:a aac -shortest final.mp4'
    else:
        cmd = f'ffmpeg -y -i voice.mp3 -stream_loop -1 -i bg_music.mp3 -f lavfi -i color=c=black:s=1080x1920:r=25 {audio_filter} -vf "drawtext=text=\'{topic}\':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -c:a aac -shortest final.mp4'
